...
if [ $slient == 0 ];then
  ...
  mkdir -p ./build
  cd ./build
  cmake -D CMAKE_BUILD_TYPE=Debug -D TARGET_ARCH=${TARGET_ARCH} ../ 2>&1 > /tmp/build_debug_cmake.log
  if [ $? -ne 0 ]; then
    cat /tmp/build_debug_cmake.log
    echo "cmake failed!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    echo ${test_file}
    exit 255
  fi
  make -j8 2>&1 > /tmp/build_debug_make.log
    if [ $? -ne 0 ]; then
      cat /tmp/build_debug_make.log
      echo "make failed!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
      echo ${test_file}
    exit 255
  fi
