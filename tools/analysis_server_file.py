...
class File_Hanlder():
    ...
    def handle(self,file_name):
        ...
        logging.debug("Accept connection from File : " + file_name)
        ...
        while 1:
            try:
                if avaiable == 0:
                    tmp_buf = self.readall(file_name)
                    if len(tmp_buf) == 0:
...
def test_analysis_server(argv,time_out,callback):
    ...
    elf_path = argv[4]
    libc_path = argv[5]
    if not os.path.exists((workspace + '/local/')):
        print 'local must exist'
        exit(-1)
    if not os.path.exists(workspace + '/raw'):
        os.mkdirs(workspace + '/raw')
    logging.info( 'start auto_tracfic_analysis_local')
    elf_info = ELFInfo(elf_path)
    libc_info = ELFInfo(libc_path)
    try:
        SocketServer.TCPServer.allow_reuse_address = True
